...
class CNNHead(nn.Module):
    def __init__(self,
        ...
        # TODO: 1.层数太少,导致图像有分层现象?;2.最后改用Tanh激活函数
        self.CNN_fuse = nn.Sequential(
                            ...
                            nn.Conv2d(in_channels=embedding_dim*6+6, out_channels=embedding_dim, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, inputs, original_input):
        ...
        # TODO: 去掉最后一层F4,下采样太大,造成白边？可能是联合训练,检测/分割损失导致最后一层下采样的问题
        F_1_4_cat = torch.cat([F1_rgb_c, F1_ir_c, F2_rgb_c, F2_ir_c, F3_rgb_c, F3_ir_c,ir_rgb_cat], dim=1)
