...
def print_in_file(fila_path):
    # 打开文件用于写入（如果文件已存在，则会被覆盖）
...
def get_p_according_train_mode(train_mode):
    p = 0
    if train_mode == 1:
        p = 0.25  # M3FD
        # p = -1  # MSRS
    elif train_mode == 2 or train_mode == 4:
        p = 0.25
    elif train_mode == 3 or train_mode == 5:
        p = -1
    elif train_mode == 6:
        p = random.random()
    return p
